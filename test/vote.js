...
contract('Vote', function (accounts) {
  beforeEach(async function () {
    token = await SweetToken.new();
    vote = await Vote.new(token.address, hero1, hero2);
    holder = accounts[1];

    await token.mint(holder, startBalance);
  });
  ...
  it('should put votes and tokens to deposit', async function () {
    let tx = await vote.staking(hero1, amount, {from: holder});

    let checkTime = (await time.latest()).toNumber();

    console.log(holder, amount, hero1, checkTime);
    ...
    let currentBallance = (await token.balanceOf(holder)).toNumber();
    ...
    assert.equal(currentBallance, startBalance - amount, 'holder should has 10000 - amount tokens');
  });
